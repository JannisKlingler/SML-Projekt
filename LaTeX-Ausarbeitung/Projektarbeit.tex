...
\begin{document}
	...
	TODO
	%Mit diesen Werten definieren wir nun die empirischen ersten Ableitungen $z_k^{(1),p} := \frac{1}{\Delta t} \, (z_{k+1}^{(0),p} - z_k^{(0),p}) \in \R^d$. Die Vorstellung als Annäherung einer Ableitung ist hier eigentlich falsch, da Lösungen von SDEs mit positivem $\bsig$ nirgends differenzierbar sind, aber wie wir schon bei der Theorie zum Lernen von SDEs gesehen hatten, ist $\E[\frac{1}{\Delta t} \, (z_{k+1}^{(0),p} - z_k^{(0),p}) \mid z_k^{(0),p}] \approx \bmu(z_k^{(0),p})$, also eignen die $z_k^{(1),p}$ sich zumindest bei großen Datensätzen zum erlernen von 
