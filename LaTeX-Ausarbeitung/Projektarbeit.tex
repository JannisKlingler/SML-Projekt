...
\end{titlepage}
	...
	Anpassungen sinnvoll. Ein praktischer Ansatz hierfür ist es, die zeitliche Komponente in den Daten zu 
	vernachlässigen und die Serie zusammengefasst dem VAE als einen Datenvektor zu übergeben. 
	Die Ergebnisse verbessern sich, wenn man die zeitlichen Zusammenhänge der Serie 
	im Latenten Raum linear interpoliert.\\
	Eine bessere Möglichkeit die häufig komplexen Zusammenhängen zwischen den Zeitschritten in \emph{time series} trotzdem zu erlernen bietet der \emph{Ordinary Differential Equation Variational Auto-Encoder} (kurz: ODE$^2$-VAE). Die zeitlichen Abhängigkeiten unter den Datenpunkten werden hier mit Differentialgleichungen zweiter Ordnung modelliert, welche im Trainingsprozess erlernt werden. \\
