...
\begin{document}
	...
	%time series noch überall klein schreiben
	...
	Wir werden daher ODEs zweiter Ordnung verwenden, welche sich äquivalent als ein System bestehend aus zwei Differentialgleichungen erster Ordung notieren lassen.\\ Der Zustand $\mathbf z_{t}=(\mathbf s_{t}, \mathbf v_{t})^{\mathrm{T}}$ zum Zeitpunkt $t$, wird dazu in eine Positionskomponente $\mathbf s_{t}$ und eine Geschwindigkeitskomponente $\mathbf v_{t}$ zerlegt, deren Anfangswerte $\mathbf s_{0}$ und $\mathbf v_{0}$ von zwei unterschiedlichen Encoder-Netzwerken, dem \emph{Position-Encoder} und dem \emph{Velocity-Encoder}, erlernt werden.\\
	...
	Ein Datenpunkt $\x\in \{0,1\}^{28\times 28\times 10}$ aus dem Datensatz enthält eine Sequenz von 10 Frames, welche von einem Frame auf das nachfolgende Frame um 36$^\circ$ gegen den Uhrzeigersinn rotieren. Insgesamt wird so eine Rotation um 360$^\circ$ dargestellt. \\
	Der Testdatensatz besteht aus 10000 Sequenzen und der Trainingsdatensatz aus 60000 Sequenzen.\\
	\textbf{TODO eventuell ein Bild einer Sequenz}
	\subsubsection{Bouncing Balls}
	Der Datensatz \emph{bouncingBalls} besteht aus Bildsequenzen, die Bewegungen dreier gleichgroßer zweidimensionaler Bälle enthalten. Die Bälle befinden sich zum Anfangszeitpunkt in zufälliger Position im Bild und bewegen sich mit normiertem Bbeschleunigungsvektor in zufällige Richtungen. Dabei können sie miteinander und den Bildrändern kollidieren. 
	Die zugrundeliegenden physikalischen Gesetze der Kollisionen zwischen den Bällen und mit den Bildrändern werden durch zweidimensionale elastische Kollisionen wie in \cite{ElastKol} beschrieben und sollen von unserem Modell erlent werden. \\
	...
	Der Testdatensatz besteht aus 10000 Sequenzen und der Trainingsdatensatz aus 60000 Sequenzen.\\
	\textbf{TODO eventuell ein Bild einer Sequenz}
	\subsubsection{AVI videosequences}
	\textbf{TODO}
	...
	Dieses Modell haben wir für die Implementierung eines VAE`s verwendet. Als Input dient hier die gesamte Videosequenz. Falls nur ein 
	einzelnen Frame betrachtet werden soll, muss im Input-Layer die Outputgröße von 10 auf 1 reduziert werden.
	...
	\end{table}
	\newpage
	Die Modellstruktur des ODE$^2$VAE auf den Datensätzen rotatingMNIST und bouncingBalls. Im ODE-Net verwenden wir als Aktivierungsfunktion den 
	...
	\end{table}
	\newpage
	Die Modellstruktur des ODE$^2$VAE auf dem Datensatz AVI-videosequences.
	\begin{table}[htb!]
		\footnotesize
		\begin{center}
			\begin{tabular}{llcccc}
