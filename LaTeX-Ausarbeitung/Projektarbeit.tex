...
\begin{document}
	...
	%Decoder nicht als Funktion auffassen.
	 $p(\tx(t,\cdot)|s(t,\cdot))$ ein guter Schätzer für $\tx(t,\cdot)$ sein. Die Anfangswerte $s(0,\cdot)$ und $v(0,\cdot)$ werden durch jeweils einen Encoder bestimmt, die als Input $x_0$, beziehungsweise mindestens drei anfangsframes für $v(0,\cdot)$, erhalten.\\
	...
	\vspace{-0.3cm}
	\textbf{TODO}
	Ein Datenpunkt $\x\in \{0,1\}^{28\times 28\times 20}$ aus dem Datensatz enthält 20 Frames.
	Der Trainingsdatensatz besteht aus 30000 Sequenzen und der Testdatensatz aus 5000 Sequenzen.
