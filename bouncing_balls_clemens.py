...
class DataGenerator(tf.keras.utils.Sequence):
    def __init__(self, object_number, picture_size, frames, dataset_size, batch_size):
    ...
    def __getitem__(self, index):
        X = create_dataset(self.batch_size, self.frames, self.picture_size, self.object_number)
        return X


#tic = time.perf_counter()
#x_train = create_dataset(100, 10, 28, 3)
#toc = time.perf_counter()
# print(np.shape(x_train))
#print(f"Datensatz erstellen braucht {toc - tic:0.4f} Sekunden")

# x_test = create_dataset(20, 10)
